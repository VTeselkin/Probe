...
void IRsend::mark(int time) {
  // Отправка сигнала на указанном пине
  digitalWrite(sendPin, HIGH);  // Устанавливаем высокий уровень
  delayMicroseconds(time);      // Ждем указанное время
  digitalWrite(sendPin, LOW);   // Возвращаем в низкий уровень
}

/* Leave pin off for time (given in microseconds) */
void IRsend::space(int time) {
  // "Пауза" (отключение сигнала) на указанном пине
  digitalWrite(sendPin, LOW);  // Убеждаемся, что уровень остается низким
  delayMicroseconds(time);     // Ждем указанное время
}
...
void IRsend::enableIROut(int khz) {
  ...
  digitalWrite(sendPin, LOW);  // Когда ШИМ не активен, установить LOW
  ...
  // Убедитесь, что выбранный пин используется
  if (sendPin != 4) {
    digitalWrite(4, LOW);  // Отключить PB4, если используется другой пин
  }
}
